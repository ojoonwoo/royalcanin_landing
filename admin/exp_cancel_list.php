...
<div id="page-wrapper">
  ...
  <!-- Page Heading -->
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">체험팩 이벤트 취소자 목록</h1>
      </div>
    </div>
    <!-- /.row -->
    <div class="row">
          ...
          <ol class="breadcrumb">
            <form name="frm_execute" method="POST" onsubmit="return checkfrm()">
              ...
              <select name="search_type">
                <option value="exp_name" <?php if($search_type == "exp_name"){?>selected<?php }?>>이름</option>
                <option value="exp_phone" <?php if($search_type == "exp_phone"){?>selected<?php }?>>전화번호</option>
              </select>
			  ...
			  <li align="right";>
			  <?
					$member = "SELECT count(idx) FROM ".$_gl['exp_info_table']." WHERE exp_name <> 'admin' AND exp_useYN='N' ";
					$res3 = mysqli_query($my_db, $member);
					list($total_count)	= @mysqli_fetch_array($res3);
					echo  "전체 취소자수 : $total_count";
				?>
			</li>
            </form>
          </ol>
...
<?php 
	...
	if ($sDate != "")
		$where	.= " AND exp_regdate >= '".$sDate."' AND exp_regdate <= '".$eDate." 23:59:59'";
	
	if ($search_txt != "")
	{
		$where	.= " AND ".$search_type." like '%".$search_txt."%'";
	}
	$buyer_count_query = "SELECT count(*) FROM ".$_gl['exp_info_table']." WHERE  exp_name <> 'admin' AND exp_useYN='N' ".$where."";
	...
	$buyer_list_query = "SELECT * FROM ".$_gl['exp_info_table']." WHERE exp_name <> 'admin' AND exp_useYN='N' ".$where." Order by idx DESC LIMIT $PAGE_CLASS->page_start, $page_size";
...
?>
              <tr>
                ...
                <td><?php echo $buyer_info[$key]['exp_name']?></td>
                <td><?php echo $buyer_info[$key]['exp_phone']?></td>
                <td>[<?php echo $buyer_info[$key]['exp_zipcode']?>] <?php echo $buyer_info[$key]['exp_addr1']?> <?php echo $buyer_info[$key]['exp_addr2']?></td>
                <td><?php echo $buyer_info[$key]['exp_level']?></td>
                <td><?php echo $buyer_info[$key]['exp_maker']?></td>
                <td><?php echo $buyer_info[$key]['exp_brand']?></td>
                <td><?php echo $buyer_info[$key]['exp_regdate']?></td>
              </tr>
<?php 
		}
	}else{
