...
$media_query	= "SELECT tracking_media, COUNT( tracking_media ) media_cnt FROM tracking_info WHERE 1 AND tracking_date LIKE '%".$todayDate."%' GROUP BY tracking_media";
...
$objPHPExcel->getProperties()->setCreator("minivertising")
							 ...
							 ->setTitle("Office 2007 XLSX Tracking Data")
							 ->setSubject("Office 2007 XLSX Tracking Data")
...
foreach($media_name as $key => $val)
{
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E'.$dataIdx, number_format($total_cnt[$i]));
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('MINIDIO_'.$todayDate);
...
header('Content-Disposition: attachment;filename="MINIDIO_'.$todayDate.'.xls"');
