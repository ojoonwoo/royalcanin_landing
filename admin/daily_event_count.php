...
<?php 
	...
	{
		$pc_query		= "SELECT * FROM ".$_gl['member_info_table']." WHERE mb_regdate LIKE  '%".$date_data['mb_date']."%' AND mb_upload_url is not null AND mb_gubun='PC'";
		$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
		$mobile_query	= "SELECT * FROM ".$_gl['member_info_table']." WHERE mb_regdate LIKE  '%".$date_data['mb_date']."%' AND mb_upload_url is not null AND mb_gubun='MOBILE'";
		$mobile_count	= mysqli_num_rows(mysqli_query($my_db, $mobile_query));
		$total_count = $pc_count + $mobile_count;
		$unique_query	= "SELECT * FROM ".$_gl['member_info_table']." WHERE mb_regdate LIKE  '%".$date_data['mb_date']."%' AND mb_upload_url is not null group by mb_ipaddr";
