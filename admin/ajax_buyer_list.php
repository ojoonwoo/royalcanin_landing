...
$where = " AND strNAME LIKE '%".$search_txt."%'";

if($search_type == "search_by_name"){
	$where = " AND strNAME LIKE '%".$search_txt."%'";
}else if($search_type == "search_by_phone"){
	$where = " AND PHONE LIKE '%".$search_txt."%'";
...
// 사용자 이름 검색 쿼리
$applicant_list_count_query = "SELECT count(*) FROM event_topgirl_main WHERE USED = '1' $where";
...
$applicant_list_query = "SELECT intseq, strNAME, strAGE, PHONE, EMAIL, ADDRESS , YOUTUBE, TYPE, LIKECOUNT, USERID, REGDATE, coupon_page, USED FROM event_topgirl_main WHERE USED = '1' $where Order by intseq DESC LIMIT $PAGE_CLASS->page_start, $page_size";
